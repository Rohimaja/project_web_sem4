{{-- <nav class="fixed z-50 xl:sticky top-0 w-full border-b border-gray-200 bg-white dark:border-gray-500 dark:bg-gray-800 py-3 px-4">
    <div class="flex justify-between items-center" 
         x-data="{ open: false, isDark: localStorage.getItem('theme') === 'dark' }"
         x-init="document.documentElement.classList.toggle('dark', isDark)">
      
      <!-- Kiri: Menu Toggle dan Role -->
      <div class="text-gray-600 dark:text-white">
        <button @click="isSideMenuOpen = !isSideMenuOpen" class="cursor-pointer block xl:hidden px-2 py-1 active:bg-gray-200 rounded-sm">
          <i class="bi bi-list font-bold text-2xl"></i>
        </button>
        <h1 class="text-lg px-2 font-semibold hidden xl:block">
      ...
      <!-- Kanan: Dark Mode, Kalender, Profile -->
      <div class="flex items-center gap-3">
        ...
        <div class="relative" x-data="{ open: false }">
          <button @click="open = !open" @click.away="open = false" class="flex items-center gap-2 focus:outline-none">
            ...
            <span class="text-gray-600 dark:text-white hidden md:inline">{{ Auth::user()->name }}</span>
