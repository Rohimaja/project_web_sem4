          ...
          <table id="tbl-pres" class="text-sm text-left w-full pt-2">
            ...
            <tbody class="text-center">
                <tr class="hover:bg-gray-50">
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->tgl_presensi ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->jam_awal .' - '.$presensi->jam_akhir ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->matkul->nama_matkul ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->dosen->nama ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->prodi->jenjang .' '.$presensi->prodi->nama_prodi ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->semester ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->ruangan->nama_ruangan ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->tahunAjaran->tahun_awal .'/'. $presensi->tahunAjaran->tahun_akhir .' '. $presensi->tahunAjaran->keterangan ?? '-'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->link_zoom ?? 'tidak tersedia'}}</td>
                    <td class="border border-gray-300 px-4 py-2">{{$presensi->link_zoom ? 'Daring' : 'Luring'}}</td>
                </tr>
            </tbody>
          </table>
        </div>
            ...
            <tbody class="text-center">
                @foreach ($detail as $dp )
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-4 py-2">{{$dp->waktu_presensi ?? '-'}}</td>
                        ...
                        <td class="border border-gray-300 px-4 py-2">{{$dp->alasan ?? '-'}}</td>


                        <td class="border border-gray-300 px-4 py-2 text-center">
                            <div x-data="{openEdit: false, status: '{{$dp->status}}'}" class="flex justify-center gap-2" x-init="openEdit = false">
                                    ...
                                    <div class="absolute inset-0 bg-black opacity-50"></div>
                                        <div @click.outside="openEdit = false" class="relative z-10 bg-white rounded-lg shadow-2xl w-[90%] max-w-md p-6">
                                            ...
                                            <div class="mb-6 w-full">
                                                ...
                                                <textarea id="alasan" name="alasan" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" x-bind:disabled="!(status == 2 || status == 3)"></textarea>
                                            </div>
                                            <div class="flex justify-end space-x-3">
                                                <button type="button" @click="openEdit = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
                                                    Batal
                                                </button>
